...
namespace App
{
    public sealed partial class AgendaPage : Page
    {
        ...
        private void CalendarControl_DayItemChanging(CalendarView sender, CalendarViewDayItemChangingEventArgs args)
        {
            ...
            ApplyDayItemStyle(args.Item);
        }

        // Controleer of er afspraken zijn op de gegeven dag, rekening houdend met de huidige filter.
        ...
        // Past de visuele markering van een kalenderdag toe op basis van de gefilterde afspraken.
        ...
        // Zoeken door de visuele boom zodat ook reeds getoonde dagcellen direct ververst kunnen worden.
        private IEnumerable<CalendarViewDayItem> FindCalendarDayItems(DependencyObject root)
        {
            if (root == null)
                yield break;
            ...
            for (int index = 0; index < childCount; index++)
            {
                ...
                if (child is CalendarViewDayItem dayItem)
                    yield return dayItem;

                foreach (var nestedDayItem in FindCalendarDayItems(child))
                {
                    yield return nestedDayItem;
                }
            }
        }

        private static void TrySetCornerRadius(CalendarViewDayItem dayItem, double radius)
        {
            try
            {
                dayItem.CornerRadius = new CornerRadius(radius);
            }
            catch
            {
            }
        ...
        private async void Button_Click_ImportIcs(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                // Post de ICS-tekst direct via de exporter en laat de status zien.
                var response = await IcsExporter.PostIcsAsync(requestUrl, icsContent);
